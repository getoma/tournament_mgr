...
class TournamentTreeController
{
   ...
   public function showMatch(Request $request, Response $response, array $args, ?TournamentStructure $structure = null, $error=null): Response
   {
      ...
      $matchList = match($ctx->pool_name)
      {
         ...
         null    => $structure->ko->getMatchList()->filter(fn($n) => $n->isReal() && $n->area === $node->area),
      };
      /** @var MatchNodeCollection $matchList */
      $current_it = $matchList->getIteratorAt($ctx->match_name);

      return $this->view->render($response, 'tournament/match/match.twig', [
         'type'     => isset($args['pool'])?'pool':'ko',
         'pool'     => $args['pool']??null,
         'node'     => $node,
         'points'   => $this->matchService->getMatchPointSummary($node),
         'previous' => $current_it->back()->current(),
         'next'     => $matchList->slice($current_it->skip()->key()),
         'error'    => $error,
      ]);
   }
