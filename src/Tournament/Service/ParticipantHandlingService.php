...
class ParticipantHandlingService
{
    ...
    * @return array update report: [ participantId -> team_id ]:
    *     [ participantId => null,    // participant doesn't exist (anymore)
    *       participantId => team_id, // participant is already part of this team and needs to be dropped from there first
    *     ]
    */
   private function updateTeamMembersAndSave(Team $team, array $participantIdList): array
   {
      ...
      foreach ($participantIdList as $pid)
      {
         ...
         {
            $prev_team = $participants[$pid]->categories[$team->category_id]?->team_id;
            if ($prev_team !== null && $prev_team !== $team->getId())
            {
